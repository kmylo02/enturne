	...
	foreach ($aspirante as $row) {
		...
		<?php if ($row->ranking >= 4.5 && $row->ranking < 5) { ?>
							...
							<?php if ($estado != 'contratado') { ?>
								<?php
								echo"<td>" . anchor_popup(base_url() . 'empresa/Perfil/generar_hv_conductor/' . $row->id, '<i class="fa fa-file-pdf-o fa-2x"></i>', array('title' => 'Ver Hoja de vida')) . "</td>";
								echo"<td>" . anchor(base_url() . 'empresa/Empleo/get_conductor_xid/' . $row->id, '<i class="fa fa-check-circle-o fa-2x"></i>', array('title' => 'Contratar')) . "</td>";
								?>
								<td><a onclick="if (confirma() == false)
																				return false" class="fa fa-trash fa-2x" href="<?php echo base_url() . 'empresa/Empleo/descartar_oferta_xid/' . $apl_id ?>" title="Descartar Aspirante"></a></td>
							<?php } else {
